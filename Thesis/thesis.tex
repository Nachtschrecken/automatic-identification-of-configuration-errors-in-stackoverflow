...
\ThesisSetSupervisors{Prof.\ Dr.\ Norbert Siegmund,M.Sc. Sebastian Simon}
...
\begin{frontmatter}
  \begin{abstract}
    Identifying configuration errors in software systems is a critical challenge, since they can lead to problems ranging from poor performance to complete system failure. Due to the increasing complexity of modern systems, errors are often caused by human factors such as lack of expertise and poor communication. Using Stack Overflow, an online developer community, as a data source to classify and analyze configuration errors, this problem can be approached. On the one hand, a machine learning model named \gls{cmpace} is used to recognize patterns in discussions about configuration errors, utilizing the Word2Vec and \gls{svm} methods. On the other hand, the large language model (LLM) \gls{gpt}-3.5 by OpenAI is employed to perform similar tasks with a focus on prompt engineering. Both approaches are compared in terms of efficiency, accuracy, and real-world applicability. Additionally, the \gls{llm} is used to analyze the dataset for configuration errors, including the type of configuration error, impact of the configuration error, and technology stack involved. The findings offer valuable insight into the applicability of utilizing different machine learning approaches, in addition to a comprehensive overview of configuration errors and their implications.
  \end{abstract}
...
\begin{table}[]
  ...
  \begin{tabular}{l|l|l}
   &
    Criterion ID &
    Description \\ \hline
  \multirow{6}{*}{\begin{tabular}[c]{@{}l@{}}Criteria for labeling \\ configuration error \\ related posts\end{tabular}} &
    \begin{tabular}[c]{@{}l@{}}C1: Description of \\ configuration error\end{tabular} &
    \begin{tabular}[c]{@{}l@{}}The description of a configuration \\ error by the author\end{tabular} \\ \cline{2-3} 
   &
    \begin{tabular}[c]{@{}l@{}}C2: Cause of \\ configuration error\end{tabular} &
    \begin{tabular}[c]{@{}l@{}}Causes that lead to a configuration \\ error\end{tabular} \\ \cline{2-3} 
   &
    \begin{tabular}[c]{@{}l@{}}C3: Approach for \\ detecting and resolving\end{tabular} &
    \begin{tabular}[c]{@{}l@{}}Methods used to detect/refactor \\ specific configuration errors\end{tabular} \\ \cline{2-3} 
   &
    \begin{tabular}[c]{@{}l@{}}C4: Tool for detecting \\ and resolving\end{tabular} &
    \begin{tabular}[c]{@{}l@{}}Tools for detecting and resolving \\ specific configuration errors\end{tabular} \\ \cline{2-3} 
   &
    \begin{tabular}[c]{@{}l@{}}C5: Impact of \\ configuration error\end{tabular} &
    \begin{tabular}[c]{@{}l@{}}Impact of a configuration error on \\ software development\end{tabular} \\ \cline{2-3} 
   &
    \begin{tabular}[c]{@{}l@{}}C6: Challenge of \\ detecting and resolving\end{tabular} &
    \begin{tabular}[c]{@{}l@{}}Challenges identified in detecting \\ and resolving configuration errors\end{tabular} \\ \hline
  \begin{tabular}[c]{@{}l@{}}Criterion for labeling \\ configuration error \\ unrelated posts\end{tabular} &
    C7: Unrelated topic &
    \begin{tabular}[c]{@{}l@{}}The post does not describe \\ configuration errors, for example \\ a post discussing optimal \\ configurations for Docker without \\ the presence of any problems\end{tabular}
  \end{tabular}
  \label{tab:table5993}
  }
  \end{table}
...
\newacronym{cmpace}{CM-PaCE}{Classification Model for Posts about Configuration Errors}
